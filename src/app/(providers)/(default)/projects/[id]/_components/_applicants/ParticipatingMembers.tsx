...
const ParticipatingMembers = ({
  ...
  return (
    <section className="w-72 mr-6">
      ...
      <article className="grid grid-cols-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 w-[1200px]">
        {participatingApplications.map((participating) => {
          return (
            <article
              key={participating.id}
              className="relative flex flex-col items-center mb-6 py-7 bg-[#F4F4F4] w-[280px] h-[400px] rounded-xl"
            >
              {isWriter && (
              ...
              participating.users?.positions?.name === "프론트엔드" ? (
                <h3 className="text-[#80E500] text-base font-semibold mt-1">
              ...
              <p className="text-[#48484A] w-[200px] whitespace-normal h-auto mt-2">
              ...
              <ul className="flex text-sm">
                {participating.users?.user_tech?.slice(0, 3).map((tech) => {
                  return (
                    <li
                      key={tech.id}
                      className="w-auto text-xs p-2 pl-3 pr-3 mt-2 mr-2 rounded-3xl text-[#2D2D2D] bg-[#E6E6E6]"
                    >
              ...
              <Link
                className="absolute text-stone-500 mt-[320px] ml-[20px]"
